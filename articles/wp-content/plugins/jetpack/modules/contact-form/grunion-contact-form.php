...
class Grunion_Contact_Form extends Crunion_Contact_Form_Shortcode {
	...
	static function parse( $attributes, $content ) {
		...
		if ( isset( $_GET['contact-form-id'] )
			...
			if ( $feedback_id && isset( $_GET['_wpnonce'] ) && wp_verify_nonce( stripslashes( $_GET['_wpnonce'] ), "contact-form-sent-{$feedback_id}" ) ) {
...
class Grunion_Contact_Form_Field extends Crunion_Contact_Form_Shortcode {
	...
	function validate() {
		...
		if ( isset( $_POST[ $field_id ] ) ) {
			if ( is_array( $_POST[ $field_id ] ) ) {
				$field_value = array_map( 'sanitize_text_field', wp_unslash( $_POST[ $field_id ] ) );
			} else {
	...
	function render() {
		...
		if ( isset( $_POST[ $field_id ] ) ) {
			if ( is_array( $_POST[ $field_id ] ) ) {
				$this->value = array_map( 'sanitize_text_field', wp_unslash( $_POST[ $field_id ] ) );
			} else {
				$this->value = sanitize_text_field( wp_unslash( $_POST[ $field_id ] ) );
			}
		} elseif ( isset( $_GET[ $field_id ] ) ) {
			$this->value = sanitize_text_field( wp_unslash( $_GET[ $field_id ] ) );
		} elseif (
			is_user_logged_in() &&
