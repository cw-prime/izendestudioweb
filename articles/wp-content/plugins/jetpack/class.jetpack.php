...
// phpcs:disable WordPress.WP.CapitalPDangit.Misspelled
	...
	public static function activate_new_modules( $redirect = false ) {
		...
		if ( $redirect ) {
			...
			if ( isset( $_GET['page'] ) && in_array( $_GET['page'], array( 'jetpack', 'jetpack_modules' ) , true ) ) {
	...
	public function throw_error_on_activate_plugin( $plugin ) {
		...
		if ( function_exists( 'stats_get_api_key' ) && in_array( 'shortlinks', $active_modules ) ) {
			...
			if ( $throw ) {
				...
				trigger_error( sprintf( esc_html__( 'Jetpack contains the most recent version of the old &#8220;%1$s&#8221; plugin.', 'jetpack' ), 'WordPress.com Stats' ), E_USER_ERROR );
			}
		}
	}
...
<div id="message" class="jetpack-message jetpack-err">
	...
	public function verify_json_api_authorization_request( $environment = null ) {
		...
		if ( self::is_redirect_encoded( esc_url_raw( wp_unslash( $_GET['redirect_to'] ) ) ) ) {
			/**
